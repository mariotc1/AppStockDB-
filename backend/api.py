...
import bcrypt, os, string, random
import pandas as pd
import logging
...
# ── Connection pool ────────────────────────────────────────────────────────────
# Un pool de 5 conexiones reutilizables evita abrir/cerrar una conexión por
# cada request, que con gunicorn multi-worker agotaría max_connections de MySQL.
_pool = pooling.MySQLConnectionPool(
    pool_name="appstockdb",
    pool_size=5,
...
@app.route('/health')
def health_check():
    """Health check que verifica también la conectividad con la base de datos."""
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify({'status': 'ok', 'service': 'AppStockDB API', 'database': 'connected'}), 200
    except Exception as e:
        logger.error("Health check: DB unreachable — %s", e)
        return jsonify({'status': 'degraded', 'service': 'AppStockDB API', 'database': 'disconnected'}), 503
...
def forgot_password():
    try:
        data = request.get_json()
        if not data:
            logger.error("JSON vacío recibido en la API")
            return jsonify({"error": "Solicitud inválida, JSON vacío."}), 400

        email = data.get('email')
        if not email:
            logger.error("No se recibió un correo en la solicitud")
        ...
        if not user:
            logger.error("El correo no está registrado: %s", email)
        ...
        logger.info("Código de recuperación generado correctamente: %s", recovery_code)
        return jsonify({"message": "Código de recuperación generado.", "recovery_code": recovery_code}), 200

    except Exception as e:
        logger.error("Excepción en forgot_password: %s", e)
...
def upload_profile_picture():
    ...
    try:
        ...
        return jsonify({'message': 'Foto de perfil actualizada', 'file_path': f"{os.getenv('API_BASE_URL', 'http://127.0.0.1:5000')}/uploads/{filename}"}), 200
    except mysql.connector.Error as err:
        return jsonify({'error': str(err)}), 500
    finally:
        cursor.close()
...
def get_user(user_id):
    try:
        ...
        if user["profile_picture"]:
            ...
            user["profile_picture"] = f"{os.getenv('API_BASE_URL', 'http://127.0.0.1:5000')}/uploads/{user['profile_picture']}"
        else:
...
def registrar_movimiento():
    data = request.get_json()
    logger.debug("Datos recibidos para historial: %s", data)
